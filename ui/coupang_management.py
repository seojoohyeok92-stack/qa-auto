...
def _models() -> list[str]:
    repo=ProductCatalogRepository(); data=repo.catalog(); aliases=data.get('aliases') or {}; keys=[*data.get('catalog',{}).keys()]
    return sorted({x for key in keys if (x:=canonical_model_identity(key,aliases=aliases))})
